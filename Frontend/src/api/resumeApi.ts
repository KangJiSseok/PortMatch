...
export const resumeApi = {
  ...
  getResumeDetail: async (resumeId: string | number): Promise<ResumeDetail> => {
    ...
    getMainResumeByUserId: async (userId: string | number): Promise<ResumeDetail> => {
        const response = await fetch(`${BASE_URL}/main/${userId}`);
        if (!response.ok) throw new Error('Failed to fetch main resume detail');
        const json: ApiResponse<ResumeDetail> = await response.json();
        if (!json.status) {
            throw new Error(json.message || '유저의 이력서가 없습니다.');
        }
        return json.data;
    },
