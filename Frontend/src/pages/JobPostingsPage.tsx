...
function JobPostingsPage() {
  ...
  const runSearch = () => {
    ...
    // companyName으로 검색하되, 실제 필터링을 위해 companyId를 찾아야 함
    // 여기서는 간단히 companyName만 저장 (향후 API 연동 시 companyId를 서버에서 찾거나 프론트에서 매칭)
    setSearchParams({ companyName: nextCompanyName, companyId, sort });
  };
  ...
  // ✅ 수정된 핵심 로직: companyId 또는 companyName으로 필터링
  const jobs = useMemo(() => {
    if (companyId) {
      // URL에서 companyId가 있으면 ID로 필터링
      return MOCK_JOBS.filter((job) => job.companyId === Number(companyId));
    }
    if (companyName) {
      // companyName으로 검색한 경우 회사명으로 필터링
      return MOCK_JOBS.filter((job) => job.company.includes(companyName));
    }
    return MOCK_JOBS; // 검색어 없으면 전체 노출
  }, [companyId, companyName]);

  // 현재 보여지는 기업명 추출
  const currentCompanyName = useMemo(() => {
    return jobs.length > 0 ? jobs[0].company : '';
  }, [jobs]);
      ...
      <div className="mx-auto max-w-5xl px-6">
        <header className="mb-12">
          <div className="mb-4 flex items-center gap-2">
            ...
            <span className="text-silver-mist text-[11px] font-black tracking-[0.2em] uppercase">
              Job Postings
            </span>
          </div>
          ...
          <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
            ...
            <div className="bg-cloud-dancer/50 flex gap-1 rounded-xl p-1">
              <button
                ...
                className={`rounded-lg px-4 py-1.5 text-[11px] font-bold transition-all ${sort === 'latest' ? 'bg-pure-white text-midnight-ink shadow-sm' : 'text-silver-mist hover:text-slate-gray'}`}
              >
                최신순
              </button>
              <button
                onClick={() => changeSort('accuracy')}
                className={`rounded-lg px-4 py-1.5 text-[11px] font-bold transition-all ${sort === 'accuracy' ? 'bg-pure-white text-midnight-ink shadow-sm' : 'text-silver-mist hover:text-slate-gray'}`}
              >
                정확도순
              </button>
            </div>
          ...
          <div className="mt-6 flex flex-col gap-3 md:flex-row md:items-center">
            <input
              ...
              placeholder="회사명 입력 (예: 삼성전자, SK하이닉스, LG전자)"
              className="border-soft-pebble bg-pure-white text-midnight-ink focus:border-midnight-ink w-full rounded-xl border px-4 py-3 text-sm font-semibold outline-none"
            />
            <button
              onClick={runSearch}
              className="bg-midnight-ink text-pure-white hover:bg-point-blue rounded-xl px-6 py-3 text-sm font-black transition-colors"
            >
              검색
            </button>
          </div>
            ...
            {jobs.length > 0 ? (
              jobs.map((job) => (
                <motion.div
                  ...
                  whileHover={{ y: -4 }}
                  className="border-cloud-dancer bg-pure-white hover:border-soft-pebble flex flex-col gap-6 rounded-[28px] border p-7 transition-all duration-300 hover:shadow-[0_10px_40px_rgba(26,26,26,0.04)] md:flex-row md:items-center"
                >
                  <div className="border-cloud-dancer bg-pure-white flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-2xl border p-2">
                    <img
                      src={job.logo}
                      alt={job.company}
                      ...
                      onError={(e) => {
                        ...
                        // 이미지가 정말 안 뜰 경우를 대비한 텍스트 대체 로직
                        if (target.parentElement) {
                          target.parentElement.innerHTML = `<span class="text-xl font-bold text-slate-gray">${job.company[0]}</span>`;
                        }
                      }}
                    />
                  </div>
                  <div className="flex-1">
                    ...
                    <p className="text-slate-gray mt-3 text-sm font-medium">{job.location}</p>
                  </div>
                  <div className="border-cloud-dancer flex shrink-0 flex-col items-start gap-3 border-t pt-5 md:items-end md:border-t-0 md:border-l md:pt-0 md:pl-10">
                    <div className="flex flex-col items-start md:items-end">
                      <span className="text-silver-mist text-[10px] font-black tracking-wider uppercase">
                        Deadline
                      </span>
                      <span
                        className={`text-lg font-black ${job.deadline === '오늘마감' ? 'text-error' : 'text-midnight-ink'}`}
                      >
                        {job.deadline}
                      </span>
                    </div>
              ...
              <EmptyState
                ...
                onAction={() => window.history.back()} // 뒤로 가기
              />
            )}
          </section>
        )}
