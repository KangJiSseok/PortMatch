...
export default function MyApplicationPage() {
  ...
  const [errorMessage, setErrorMessage] = useState('지원 목록을 불러오지 못했어요.');
  ...
  const load = async () => {
    setIsLoading(true);
    setIsError(false);
    setErrorMessage('지원 목록을 불러오지 못했어요.');
  ...
  const handleCancel = async (jobPostingId: number) => {
    ...
    try {
      ...
      setNotice(err instanceof Error ? err.message : '지원 취소 중 오류가 발생했어요.');
    } finally {
  ...
  return (
    <div className="text-midnight-ink min-h-screen bg-[#FBFCFE] pt-32 pb-32">
        ...
        <section className="space-y-8">
          ...
          {!isLoading && !isError && (
            <div className="grid gap-4">
              {items.length === 0 ? (
                <div className="rounded-[28px] border border-dashed border-zinc-200 bg-white py-16 text-center">
                  <p className="text-sm font-bold text-zinc-500">표시할 지원 내역이 없습니다.</p>
                </div>
              ) : (
                items.map((item) => {
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-4 flex items-center gap-4 text-zinc-500">
                          ...
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="max-w-[220px] truncate text-xs font-bold">
                              {item.resumeTitle}
                            </span>
                          </div>
                        </div>
                      ...
                      {canCancel && (
                        <div className="absolute top-2 right-2">
                          <Button
                            ...
                            title="지원 취소"
                            className="border-none bg-transparent !text-rose-600 shadow-none transition-all duration-200 hover:scale-110 hover:bg-transparent hover:!text-zinc-400 active:scale-95"
                          >
                            {isCanceling ? (
      ...
      {notice && (
        <div className="bg-midnight-ink fixed bottom-12 left-1/2 z-50 -translate-x-1/2 rounded-2xl px-8 py-4 text-sm font-black text-white shadow-2xl">
          {notice}
        </div>
      )}
    </div>
